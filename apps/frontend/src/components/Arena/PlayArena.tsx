    ...
    <div className="min-h-screen flex flex-col p-4 md:p-8 relative overscroll-none">
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10 z-10 relative">
            ...
            <div className="flex flex-col gap-3 h-full max-h-[calc(100vh-2rem)] overflow-hidden">
                                        ...
                                        return (
                                            <div key={i} className="grid grid-cols-8 text-[11px] font-mono rounded-xl overflow-hidden border border-white/5 bg-white/[0.015] hover:bg-white/[0.04] transition-colors">
                                                ...
                                                <div 
                                                   ...
                                                   className={`col-span-3 flex items-center px-3 py-2 font-bold transition-colors cursor-pointer border-r border-white/5 ${currentMoveIndex === wIndex ? 'bg-blue-500 text-white' : 'text-slate-100 hover:text-blue-400'}`}
                                                >
                                                    {wMove}
                                                </div>
                                                <div 
                                                   onClick={() => goToMove(bIndex)}
                                                   className={`col-span-4 flex items-center px-3 py-2 transition-colors cursor-pointer ${!bMove ? '' : currentMoveIndex === bIndex ? 'bg-blue-500 text-white font-bold' : 'text-slate-400 hover:text-blue-400'}`}
                                                >
                                                    {bMove || ""}
                                                </div>
                                            </div>
                ...
                {/* PGN Actions — always visible when game is over */}
                {gameOver && (
